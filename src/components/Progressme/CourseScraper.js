...
const CourseScrapper = () => {
  ...
  const urlRegex = /^https?:\/\/(?:www\.)?(?:new\.)?(?:progressme\.ru|edvibe\.com)\/(?:sharing-material|SharingMaterial)\/[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}(\/book\/\d+)?$/

  const targetUrlSet = () =>(
      targetUrl.match(urlRegex)?.[0]?.length || false
  );


  const handleClick = (e, setLinkLoading) => {
      e.preventDefault();
      
      // console.log(e.target?.[0]?.value);
      setLinkLoading(true)
    
      let url = (e.target?.[0]?.value.trim() || "")?.replace("new.","").replace("edvibe.com","progressme.ru").replace("sharing-material", "SharingMaterial").replace("course", "SharingMaterial").replace(/\/book\/[0-9]{6}/,"") ;
      
      if(!url.toString().match(urlRegex)?.[0].length){
        dispatch(addError("book link is not correct. Please check: "))
        console.log("buggy url",url)
        url = testUrl;
      }
      // console.log("url:", url);
  ...
  useEffect(() => {
    ...
    const handleAuth = async(e, setAuthing)=>{
      ...
      const PulsingLoading=()=>{
        ...
        const pulses = new Array(5).fill(<Pulse />)
        return(
          <div>
            {
              pulses.map((p,i)=>(
      ...
      const MemoizedIframe = memo(
        ({authedIn,targetUrlSet, linkLoading, ...props }) => {
        ...
        <Form id="load-link" className='w-75 ' onSubmit={(e)=> handleClick(e, setLinkLoading)}>
          <FormGroup className='w-100' >
            <Input
              ...
              // pattern={urlRegex}
              // onInvalid={(e)=>{e.preventDefault(); return e.currentTarget.classList.toggle("is-invalid")}}
              ...
              <Card className='h-100'>
                ...
                <CardBody className='p-4 h-100'  >
                  <Row className='h-100 w-100 overflow-hidden' >
                    <LeftComponent authedIn={isAuthed} targetUrlSet={targetUrlSet()}/>
                    <Col sm="3"> 
                    {
                      isAuthed ? (
...
export default memo(CourseScrapper);