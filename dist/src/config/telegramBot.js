...
class TelegramBot {
    ...
    initialiseBot() {
        ...
        // Get Token Command
        this.bot.command('get_token', async (ctx) => {
            const chatId = ctx.chat?.id?.toString();
            if (!chatId) { // Handle undefined chatId
                return ctx.reply('Could not determine your chat ID. Please try again.');
            }
            ...
            if (registrationRequest.length > 0 && registrationRequest[0].status === 'approved') {
                if (registrationRequest[0].apiToken && registrationRequest[0].apiToken !== 'null') {
                    const message = await ctx.reply(Format.fmt(Format.bold(`Here is your access token:\\n`), Format.spoiler(Format.fmt(Format.code(registrationRequest[0].apiToken))), Format.quote(`\\n*IMPORTANT*: Do **NOT** share this token with anyone else.`)), Markup.inlineKeyboard([
                    ...
                    ctx.reply(`You have not completed the progressme login step.\\nPlease provide your progressme password to complete.`);
                }
            }
            else {
        ...
        this.bot.action('dashboard_user_info', async (ctx) => {
            await ctx.answerCbQuery();
            ctx.reply('User Basic Info: (Placeholder for backend data)');
        });
        this.bot.action('dashboard_num_classes', async (ctx) => {
            await ctx.answerCbQuery();
            ctx.reply('Number of Classes: (Placeholder for backend data)');
        });
        this.bot.action('dashboard_num_students', async (ctx) => {
            await ctx.answerCbQuery();
            ctx.reply('Number of Students: (Placeholder for backend data)');
        });
        this.bot.action('copy_to_clipboard', async (ctx) => {
            await ctx.answerCbQuery('Token copied to clipboard (manually select and copy from the message above).');
        });
        this.bot.action('regenerate_api_token', async (ctx) => {
            const chatId = ctx.chat?.id;
            await ctx.answerCbQuery('Initiating token regeneration...');
            try {
                ctx.editMessageText('Please confirm you want to delete the old token and generate a new token.', Markup.inlineKeyboard([
