    ...
    bot.use(async (ctx, next) => {
        ...
        if (state === 'awaiting_email') {
            ...
            if (/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/.test(text)) {
                ...
                try {
                    const response = await axios.post(`${env.SERVER_URL}/api/telegram/register-request`, {
