...
import { courses, courseBlocks, courseCorrections } from '../../drizzle/schema'; // Import Drizzle schemas
...
courseRoutes.post('/', auth, validator("json", (value, c) => {
...
courseRoutes.get('/', auth, async (c) => {
    try {
        ...
        const db = c.env.drizzleDb;
        const coursesList = await db.select().from(courses).where(eq(courses.userId, user.id)).orderBy(desc(courses.lastAccessed));
...
courseRoutes.get('/:id', auth, async (c) => {
    try {
        ...
        const db = c.env.drizzleDb;
        const course = await db.select().from(courses).where(and(eq(courses.id, courseId), eq(courses.userId, user.id)));
        if (course.length === 0) {
 ...
 *         description: Server error
 */
courseRoutes.put('/:id', auth, validator("json", (value, c) => {
    ...
    try {
        ...
        await db.update(courses).set({ progress, lastAccessed: new Date().toISOString() }).where(eq(courses.id, courseId));
        const updatedCourse = await db.select().from(courses).where(eq(courses.id, courseId));
        return c.json(updatedCourse[0]);
    }
    catch (err) {
        console.error(err.message);
        return c.json({ msg: "Server Error" }, 500);
 ...
 *         description: Server error
 */
courseRoutes.post('/:courseId/blocks', auth, validator("json", (value, c) => {
    ...
    try {
        ...
        return c.json({ msg: 'Content block added successfully', block: newBlock[0] }, 201);
    }
    catch (err) {
        console.error(err.message);
        return c.json({ msg: "Server Error" }, 500);
 ...
 *         description: Server error
 */
courseRoutes.put('/:courseId/blocks/:blockId', auth, validator("json", (value, c) => {
    ...
    try {
        ...
        const block = await db.select().from(courseBlocks).where(and(eq(courseBlocks.id, blockId), eq(courseBlocks.courseId, courseId)));
        if (block.length === 0) {
            return c.json({ msg: 'Content block not found' }, 404);
        }
        await db.update(courseBlocks).set({ type: type || block[0].type, content: content || block[0].content, order: order || block[0].order }).where(eq(courseBlocks.id, blockId));
        const updatedBlock = await db.select().from(courseBlocks).where(eq(courseBlocks.id, blockId));
        return c.json({ msg: 'Content block updated successfully', block: updatedBlock[0] });
    }
    catch (err) {
        console.error(err.message);
        return c.json({ msg: "Server Error" }, 500);
...
courseRoutes.delete('/:courseId/blocks/:blockId', auth, async (c) => {
    ...
    try {
        ...
        await db.delete(courseBlocks).where(eq(courseBlocks.id, blockId));
 ...
 *         description: Server error
 */
courseRoutes.post('/:courseId/corrections', auth, validator("json", (value, c) => {
 ...
 *         description: Server error
 */
courseRoutes.put('/:courseId/corrections/:correctionId', auth, validator("json", (value, c) => {
    ...
    try {
        ...
        const correction = await db.select().from(courseCorrections).where(and(eq(courseCorrections.id, correctionId), eq(courseCorrections.courseId, courseId)));
        if (correction.length === 0) {
            return c.json({ msg: 'Correction not found' }, 404);
        }
        await db.update(courseCorrections).set({ correctionText: correctionText || correction[0].correctionText, blockId: blockId || correction[0].blockId }).where(eq(courseCorrections.id, correctionId));
        const updatedCorrection = await db.select().from(courseCorrections).where(eq(courseCorrections.id, correctionId));
        return c.json({ msg: 'Correction updated successfully', correction: updatedCorrection[0] });
    }
    catch (err) {
        console.error(err.message);
        return c.json({ msg: "Server Error" }, 500);
...
courseRoutes.delete('/:courseId/corrections/:correctionId', auth, async (c) => {
    ...
    try {
        ...
        await db.delete(courseCorrections).where(eq(courseCorrections.id, correctionId));
