...
class CourseScraperService {
        ...
        try {
            ...
            while (retryCount < maxRetries) {
                try {
                    ...
                    await new Promise((resolve) => setTimeout(resolve, Math.min(2000 * Math.pow(2, retryCount), 20000)));
                }
            }
        }
        catch (error) {
    ...
    async authenticateWithWebSocket(email, password) {
        try {
            ...
            return new Promise((resolve, reject) => {
                ...
                // console.log("loginMessage", JSON.stringify(loginMessage));
                ...
                ws.on("message", async (data) => {
                    ...
                    if (response.Method === "Login") {
                        if (response.IsSuccess) {
                            ...
                            reject(new Error("Login failed: " + response?.Message || "credentials error"));
                        }
                    }
                });
                ws.on("error", (error) => {
    ...
    async getIdMaterial(code) {
        try {
            ...
            return new Promise((resolve, reject) => {
                ws.send(JSON.stringify(this.controllerTemplates.GetIdMaterialMessage(code)));
                ws.on("message", async (data) => {
                    const response = JSON.parse(data.toString());
                    if (response.Class === "SharingMaterialWsController" &&
    ...
    async getBookById(bookId) {
        try {
            ...
            return new Promise((resolve, reject) => {
                ws.send(JSON.stringify(this.controllerTemplates.GetBookMessage(`\"${bookId}\"`)));
                ws.on("message", async (data) => {
                    const response = JSON.parse(data.toString());
                    if (response.Class === "SharingMaterialWsController" &&
    ...
    async getBookByCode(bookCode) {
        try {
            ...
            return new Promise((resolve, reject) => {
                const getBookIdStringed = JSON.stringify(this.controllerTemplates.GetIdMaterialMessage(`\"${bookCode}\"`));
                ...
                ws.on("message", async (data) => {
                    ...
                    if (response.Class === "SharingMaterialWsController" &&
                        ...
                        ws.send(JSON.stringify(this.controllerTemplates.GetBookMessage(`\"${bookInfo.bookId}\"`)));
    ...
    async isCanSharingMaterial(bookId, userId, token) {
        try {
            ...
            const ws = await this.createWebSocketConnection(wsUrl);
            return new Promise((resolve, reject) => {
                const checkIfCanShareMessage = JSON.stringify(this.controllerTemplates.IsCanSharingMaterialMessage(bookId, userId));
                ...
                ws.on("message", async (data) => {
                    ...
                    if (response.Class === "BookWsController" &&
    ...
    async setSharingMaterialId(bookId, token) {
        try {
            ...
            const ws = await this.createWebSocketConnection(wsUrl);
            return new Promise((resolve, reject) => {
                const getSharingMaterialIdMessage = JSON.stringify(this.controllerTemplates.GetSharingMaterialMessage(bookId));
                console.log("getSharingMaterialIdMessage", getSharingMaterialIdMessage);
    ...
    async copyCourse(bookId, userId, token) {
        try {
            ...
            const ws = await this.createWebSocketConnection(wsUrl);
            return new Promise((resolve, reject) => {
                const copyBookStringed = JSON.stringify(this.controllerTemplates.CopySharedBookMessage(bookId, this.currentBook.sharingMaterialId));
    ...
    async authenticateWithProgressMe(email, password) {
        try {
            ...
            const fieldsFound = await newPage.evaluate(() => {
                ...
                console.log("btn:", document.querySelector('button[data-bind="click:GetRoles"]')
                    ?.tagName);
                return (document.querySelector('input[name="Email"]')?.type !==
                    null &&
                    document.querySelector('input[name="Password"]')?.type !==
                        null &&
                    document.querySelector('button[data-bind="click:GetRoles"]') !== null);
            });
            //// Fill in login form
    ...
    async handleCaptcha(page = this.page) {
        try {
            ...
            const points = this.generateMousePath({ x: 0, y: 0 }, { x: box.x + box.width / 2, y: box.y + box.height / 2 });
